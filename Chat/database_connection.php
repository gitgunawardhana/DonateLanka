<?php
  $connect = new PDO("mysql:host=localhost:3308;dbname=donate_lanka;charset=utf8mb4", "root", "");
  ...
  function get_user_name($user_id, $connect){
    $query = "SELECT first_name FROM requester WHERE nic = '$user_id'";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetchAll();
    ...
    if ($result != null) {
      ...
      $query = "SELECT first_name FROM donor WHERE nic = '$user_id'";
      $statement = $connect->prepare($query);
      $statement->execute();
      $result = $statement->fetchAll();
