  ...
  // get next requester id - end
    ...
    move_uploaded_file($_FILES['profilePicture']['tmp_name'], $dst);
      ...
      $password = password_hash($password, PASSWORD_DEFAULT);
			$insert1="INSERT INTO `requester` (nic,first_name,last_name,gender,age,occupation,profile_picture,mobile,email,address,username,password) VALUES ('$nic','$first_name','$last_name','$gender','$age','$occupation','$dst_db','$mobile','$email','$address','$username','$password')";

			$query = mysqli_query($con, $insert1) or die(mysqli_error($con));
			if($query == 1){	
        setcookie('message', "Registration Successful !", time()+3);
        header("location:./registrationRequester.php");	
...
<body>
        ...
        <ul class="navbar-nav ml-auto mb-2 mb-lg-0" <?php if($_SESSION['usertype'] != "user"){ ?> style="margin-right: 20px;" <?php } ?>>
          ...
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" id="navbarDarkDropdownMenuLink" role="button" data-bs-toggle="dropdown" aria-expanded="false">
              <?php if($_SESSION["usertype"] == "admin") {?> Add User <?php }elseif($_SESSION["usertype"] == "requester") {?> Register <?php }else {?> Register <?php } ?>
            </a>
  ...
  <div class="container custom-container">
    ...
    ?>
    <h2 style="padding-bottom: 20px;"><b>Requester Registration</b></h2>
